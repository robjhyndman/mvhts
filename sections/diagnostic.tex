...
\end{equation}
the other variables' incoherences add nothing: $\bm{B}_{jk} = \bm{0}$ for all $k \ne j$. This is a standard linear hypothesis in a multivariate regression with $n_b$ responses and $(m-1)n_a$ restrictions per response. We test it with Wilks' $\Lambda$, using Rao's $F$ approximation to its null distribution \citep{rao1951asymptotic}, applied to the one-step residuals of the base models. With $m$ variables there are $m$ such tests, which we combine with a Bonferroni correction. The test uses the sample covariance, so it is unaffected by the shrinkage that biases $\widehat\kappa$ and $\widehat\gamma$, and it needs $T > n_b + m n_a + 1$.

We also tried calibrating $\widehat\kappa$ and $\widehat\gamma$ by bootstrap, generating null data from a covariance that satisfies Proposition~\ref{prop:equivalence} and is fitted to the data. Such procedures were badly mis-sized when the number of series is large relative to $T$, because any null covariance built from $\widehat{\bm{W}}$ inherits its estimation error, and the regression test avoids simulation altogether. Figure~\ref{fig:power} reports its size and power for the covariance families of Section~\ref{sec:simulation}. Under both separable and separable-plus-invisible covariances the rejection rate at the 5\% level lies between \testSizeRange\% at every sample size, in both hierarchies. Power grows with the size of the departure and with $T$; the smallest departures, with population gains below 1\%, are detected reliably only with around a thousand observations.
