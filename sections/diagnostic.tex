...
We also tried calibrating $\widehat\kappa$ and $\widehat\gamma$ by bootstrap, generating null data from a covariance that satisfies Proposition~\ref{prop:equivalence} and is fitted to the data. Such procedures were badly mis-sized when the number of series is large relative to $T$, because any null covariance built from $\widehat{\bm{W}}$ inherits its estimation error, and the regression test avoids simulation altogether. Figure~\ref{fig:power} reports its size and power for the covariance families of Section~\ref{sec:simulation}. \todo{summarise size and power, from power results}
